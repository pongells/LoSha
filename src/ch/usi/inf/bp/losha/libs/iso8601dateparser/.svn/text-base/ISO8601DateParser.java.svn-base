package ch.usi.inf.bp.losha.libs.iso8601dateparser;
...
public class ISO8601DateParser {
	...
	public static Date parse(String input) throws java.text.ParseException {
		...
		if (input.endsWith("Z")) {
			...
			final String s1 = input.substring(input.length() - inset,
					input.length());
	...
	public static String toString(final Date date) {
		...
		final String output = df.format(date);

		final int inset0 = 9;
		final int inset1 = 6;

		final String s0 = output.substring(0, output.length() - inset0);
		final String s1 = output.substring(output.length() - inset1, output.length());

		String result = s0 + s1.replaceAll("\\.", ":");
